 ...
 * Author:     Juan Celada
 * Modifier:    Dieter de Wit
 *
 * Assignment: W14 - EJERCICIO martes Operaciones con Vectores
 *
...
void Allocate_vectors(double** local_x_pp, double** local_y_pp,
      double** local_z_pp, double** local_m_pp, int local_n, MPI_Comm comm);
...
void Print_vector(double local_b[], int local_n, int n, char title[], 
      int my_rank, MPI_Comm comm);
void Parallel_vector_sum(double local_x[], double local_y[], double local_z[],
                         int local_n, double* max, MPI_Comm comm);

// Funcion utilizada para normalizar el vector
void Normalizar(double local_z[], double local_m[], int local_n, double max);
...
#  endif
   Allocate_vectors(&local_x, &local_y, &local_z, &local_m, local_n, comm);
   ...
   start = MPI_Wtime();
   Parallel_vector_sum(local_x, local_y, local_z, local_n, &max, comm);
   ...
   stop = MPI_Wtime();
   totalTime = stop-start;
   if (my_rank == 0) printf("El mayor de los elementos del vector es: %f\n", max);
   Normalizar(local_z, local_m, local_n, max);
   Print_vector(local_m, local_n, n, "La normalizacion es de", my_rank, comm);
   stop = MPI_Wtime();
   totalTime = stop-start;
   printf("Tiempo Normalizacion: %f \n", totalTime);
...
void Allocate_vectors(
   ...
   *local_m_pp = malloc(local_n*sizeof(double));
...
/*-------------------------------------------------------------------
 * Function:  Normalizar
 * Purpose:   resultado de dividir el valor del elemento dentro del  máximo
 */
void Normalizar(double local_z[], double local_m[], int local_n, double max) {
    int count;

    // resultado de dividir el valor del elemento dentro del  máximo
    for (count = 0; count < local_n; count++) {
        local_m[count] = local_z[count]/max;
    }
}  /* Normalizar */
...
void Parallel_vector_sum(
   ...
   for (local_i = 0; local_i < local_n; local_i++) {
      ...
      MPI_Reduce(&local_z[local_i], max, 1, MPI_DOUBLE, MPI_MAX, 0, comm);
   }
}  /* Parallel_vector_sum */
